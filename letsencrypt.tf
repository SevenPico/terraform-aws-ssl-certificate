...
resource "tls_cert_request" "this" {
  ...
  subject {
    common_name =  local.create_letsencrypt_csr_only ? var.common_name_override : (var.create_wildcard ? "*.${module.context.domain_name}" : module.context.domain_name)
  }
}

resource "acme_certificate" "this" {
  count = module.letsencrypt_context.enabled && !local.create_letsencrypt_csr_only ? 1 : 0
