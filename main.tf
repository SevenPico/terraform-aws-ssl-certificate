locals {
  ...
  imported_secret_certificate       = jsondecode(one(data.aws_secretsmanager_secret_version.imported[*].secret_string))[var.certificate_keyname]
  imported_secret_certificate_chain = jsondecode(one(data.aws_secretsmanager_secret_version.imported[*].secret_string))[var.private_key_keyname]
  imported_secret_private_key       = jsondecode(one(data.aws_secretsmanager_secret_version.imported[*].secret_string))[var.certificate_chain_keyname]
